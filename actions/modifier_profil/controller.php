...
<div class="content_box">
	...
	if (isset($_POST['modif'])){
		if($_POST['modif']==1){
			if(!empty($_POST['nom'])&&!empty($_POST['prenom'])){
				...
				Tracabilite($_POST['prenom']." ".$_POST['nom']." vient de modifier son profil");
				$_POST['modif']=0;
				$text_valide = '<p class="modifReussie">Modifcation réussie !</p>';
			}
			if(!empty($_POST['mdpactuel'])){
				$requete_mdp = $SQL->ExecuteSQL("select password from profil where id_profil='$id_profil'");
				$mdpactuel = $SQL->ResSQL($requete_mdp);
				if($_POST['mdpactuel']==$mdpactuel->password){
					if(($_POST['mdp1']==$_POST['mdp2'])&&(!empty($_POST['mdp1'])&&!empty($_POST['mdp2']))){
						modifier_mdp($id_profil, $_POST['mdp1']); 
						Tracabilite($_POST['prenom']." ".$_POST['nom']." vient de modifier son mot de passe");
					}
					else{
	...
	<?php
		input("Pass actuel :","mdpactuel","password","");
