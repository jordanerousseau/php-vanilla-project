	...
	if($lastpage > 1){
		...
		if ($this->_page > 1){
			if((isset($_GET['action']))&&(!empty($_GET['action']))){
				$pagination.= "<a href='".$path."action=".$_SESSION['action']."&$this->_instance=$prev"."$ext'><< précédent</a>";
			}
			else{
		...
		if ($lastpage < 7 + ($adjacents * 2)){
			for ($counter = 1; $counter <= $lastpage; $counter++){
				if ($counter == $this->_page)
					...
					if((isset($_GET['action']))&&(!empty($_GET['action']))){
						$pagination.= "<a href='".$path."action=".$_SESSION['action']."&$this->_instance=$counter"."$ext'>$counter</a>";
					}
					else{
		...
		if ($this->_page < $counter - 1){
			if((isset($_GET['action']))&&(!empty($_GET['action']))){
				$pagination.= "<a href='".$path."action=".$_SESSION['action']."&$this->_instance=$next"."$ext'>suivant >></a>";
			}
			else{
